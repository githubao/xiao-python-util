...
def encode_decode():
    ...
    uni_bin =  ' '.join([bin(int(item,base=16)).replace('0b','') if item else '' for item in str(byte_uni).lstrip('b\'\\u').rstrip('\'').split('\\\\u')])
    utf8_bin =  ' '.join([bin(int(item,base=16)).replace('0b','') if item else '' for item in str(byte_utf8).replace('\\x','\\\\x').lstrip('b\'\\x').rstrip('\'').split('\\\\x')])
    gbk_bin =  ' '.join([bin(int(item,base=16)).replace('0b','') if item else '' for item in str(byte_gbk).replace('\\x','\\\\x').lstrip('b\'\\x').rstrip('\'').split('\\\\x')])
    ...
    print('i am {{{}}}. my unicode is: {{{}}}, i am equal to it.\nmy binary code is {{{}}}. this is my actual look in memory.'.format(str_uni, str_raw,uni_bin))
    print('use unicode to print, i am {{{}}}. use utf8 to print, i am {{{}}}. use gbk to print, i am {{{}}}.'.format(str_uni,str_utf8,str_gbk))
    print('i am just 0,1 bits in memory. so for human readable, i have to be interpreted to specific format. just like utf8,gbk.'.format(uni_bin))
    ...
    print('as a binary, so when i am in utf8 format, if you use gbk to interpret me, i will be messy.')

    print()

    print('i am {{{}}}. i am unicode, not be interpreted, now i am byte type : {{{}}}.'.format(str_raw, unicode_use_uni_to_byte))
    print('i am {{{}}}. i am unicode, use utf8 to interpret,now i am byte type: {{{}}}.'.format(str_raw, unicode_use_utf8_to_byte))
    print('i am {{{}}}. i am unicode, use gbk to interpret,now i am byte type: {{{}}}.'.format(str_raw, unicode_use_gbk_to_byte))

    print()

    print('i am {{{}}}. i am byte in unicode format, now i am unicode type:{{{}}}. '.format(byte_uni, byte_in_uni_format_to_unicode))
    print('i am {{{}}}. i am byte in utf8 format, now i am unicode type:{{{}}}. '.format(byte_utf8, byte_in_utf8_format_to_unicode))
    print('i am {{{}}}. i am byte in gbk format, now i am unicode type:{{{}}}. '.format(byte_gbk, byte_in_gbk_format_to_unicode))

    print()

    print('for human readable, i used space in bin format!')
